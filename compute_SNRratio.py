...
for n_star, this_star in enumerate(star_name):
    ...
    plt.axhline(Mlimit, c='g', label='Lower mass limit on transit observability')

    plt.axvline(cop_P, c='c', label='Coplanar transit upper limit')
    plt.axvline(d05_P, c='m', label='i+5 deg transit upper limit', linestyle='dotted')
    plt.axvline(d10_P, c='y', label='i+10 deg transit upper limit',  linestyle='dashed')
    CS = plt.contour(Period, Mass, K, V)
